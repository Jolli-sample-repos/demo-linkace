...
| Public | Public works the same way like internal. If guests are additionally able to view stuff set to public depends on the [Guest Mode](/docs/v2/configuration/system-settings/#guest-settings). If Guest Mode is enabled, guests can view anything that is set to public. |
...
LinkAce must be able to send emails to invite new users. Please refer to the [email configuration docs](/docs/v2/setup/post-setup/) for more details.
...
If a user is deleted, the user profile will be disabled. However, all content the user created and has set to public or internal can be used by other users. A deleted user can be restored at any point.