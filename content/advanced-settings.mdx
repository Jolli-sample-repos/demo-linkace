...
{/* Build timestamp: 1766438826258 - Forces cache invalidation */}
...
# Set the time after a session expires automatically, in minutes. Default is 7 days (10080 minutes). You may also define a specific domain and session store.
...
# If you are using self-signed certificates, set this setting to false to disable verification (not recommended for production)
...
APP_USER_AGENT="LinkAce/2 (https://github.com/Kovah/LinkAce)"
...
# Enable backups here (enabled by default)
BACKUP_ENABLED=true
# Choose the destination of the backup. If you set up AWS S3 credentials below you may choose 's3' which is used
# for AWS. Leave blank or set to 'local_backups' if you want to store backups within /storage/app/backups.
...
# Maximum size of the backups in megabytes (default is 265 MB)
BACKUP_MAX_SIZE=265
# Backup tasks are run nightly every day. You can change the hour of the executions here (24-hour format)
BACKUP_CLEAN_HOUR="01:00"
BACKUP_RUN_HOUR="02:00"
# Optional password for backup archive encryption
...
# SSO OIDC JWT verification (added in v2.3.0)
# Set to false to disable JWT verification for OIDC providers (not recommended for production)
SSO_OIDC_VERIFY_JWT=true

---

## You probably do not want to change any values below. Only continue if you know what you are doing.
## If you are using Redis or Memcache, you may switch SESSION_DRIVER and CACHE_DRIVER to "redis" or "memcached" to improve performance.
SESSION_DRIVER=file
LOG_CHANNEL=stack
