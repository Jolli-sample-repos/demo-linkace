...
# If you are using self-signed certificates, set this to false to disable SSL verification (not recommended for production)
...
# for AWS S3-compatible storage. Leave blank or set to 'local_backups' if you want to store backups within /storage/app/backups.
BACKUP_DISK=s3
...
# Configure OIDC SSO JWT verification (added in v2.3.0)
