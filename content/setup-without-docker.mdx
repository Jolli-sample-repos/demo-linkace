...
job:
  steps:
    - name: "Update Article"
      run_prompt: |
        check out the repo and see the last change both summary and details, if the article contains factual differences with what has just changed in the repository, make edits via the add/delete/update_section tool 
...
* **PHP 8.1, 8.2, 8.3, 8.4, or 8.5** (actively tested versions), with the following extensions
  * BCMath
  * Ctype
  * DOM
  * Fileinfo
