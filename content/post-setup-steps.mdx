...
Please add the following block to your .env file and change the values according to your needs:
...
| MAIL_FROM_ADDRESS | any email address | hello@example.com | The email address you are sending from |
| MAIL_FROM_NAME | any string | LinkAce | The name you are sending from |
| MAIL_MAILER | log, smtp, sendmail, mailgun, ses, postmark | smtp | The method of sending email, more details see the Mail configuration docs |
| MAIL_HOST | any string | smtp.mailgun.org | SMTP only: the host sending email from |
...
The cron is necessary to enable automated backups via the Wayback Machine, regular link checks and application backups. You will be presented a cron token and a pre-built cron URL on the system settings page, available from the username dropdown.

Your cron must run every minute and must open the URL provided in the system settings. More information about how to configure a cron can be found in the System Settings.

## 3. Configure basic app settings

On the user settings page you can find basic app settings like the timezone, date and time format, and some privacy defaults. I recommend setting the correct timezone and the date and time formats too. You may also set which share buttons should be available when viewing your links.
