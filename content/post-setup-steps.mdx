...
| MAIL_MAILER | log, smtp, sendmail, mailgun, ses, postmark, failover | log | The method of sending email, more details see the Mail configuration docs |
...
The cron is necessary to enable automated backups via the Wayback Machine, enabling regular link checks and application backups. You will be presented with a cron token and a pre-built cron URL on the system settings page, which is available from the username dropdown. More information about how to configure a cron can be found in the System Settings.

## 3. Configure basic app settings

On the user settings page (available from the dropdown menu beneath your username), you can find basic app settings like the timezone, date and time format, and privacy defaults. I recommend setting the correct timezone and the date and time formats. You may also configure which share buttons should be available when viewing your links.
