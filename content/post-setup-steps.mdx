...
**The cron is necessary to enable automated backups via the Wayback Machine, enabling regular link checks and application backups.** You will be presented a cron token, and a pre-built cron URL on the system settings page, available from the admin dropdown menu. More information about how to configure a cron can be found in the [System Settings](https://www.linkace.org/docs/v2/configuration/system-settings/).

The cron performs the following tasks:
- Processes queued jobs (imports, Internet Archive backups)
- Checks links for availability every 2 hours
- Runs application backups (if enabled) at the configured times
